...
export default function NurseDoctorSchedulePage({
  params,
}: {
  ...
  const fetchSchedule = async (dateValue: string) => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/nurse/doctors/${doctorId}/schedule?date=${dateValue}`,
        {
          headers: {
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, opacity: 0.75 }}>
              Slots, appointments, and workload overview.
            </div>
          </div>
          <Link
            href="/nurse/doctors"
