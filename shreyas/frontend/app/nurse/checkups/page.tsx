...
const STATUS_OPTIONS = ["pending", "completed", "cancelled", "missed"];
const PRIORITY_OPTIONS = ["normal", "high", "low"];

export default function NurseCheckupsDashboard() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      if (!token) return;
      const res = await fetch(`${API_BASE}/nurse/checkups/departments`, {
        headers: {
  ...
  const fetchDoctors = async (department: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/nurse/checkups/doctors?department=${encodeURIComponent(
          department,
        )}`,
        {
          headers: {
  ...
  const fetchCheckups = async (state: FilterState) => {
    try {
      setLoading(true);
      setError(null);
      const token = getAuthToken();
      ...
      const url = params.toString()
        ? `${API_BASE}/nurse/checkups?${params.toString()}`
        : `${API_BASE}/nurse/checkups`;
      ...
      if (!res.ok) {
        setError("I couldn't load checkups right now.");
        setCheckups([]);
        return;
      }
      const data = (await res.json()) as CheckupItem[];
      setCheckups(data);
    } catch {
  ...
  const updateCheckup = async (
    ...
    try {
      ...
      const endpoint =
        action === "complete"
          ...
          : action === "cancel"
            ...
            : `/nurse/checkups/${checkupId}/priority`;
      ...
      if (!res.ok) {
        setError("I couldn't update that checkup.");
        return;
      }
      fetchCheckups(filters);
    } catch {
      setError("I couldn't reach the server. Please try again.");
    }
  };
  ...
  const statusBadge = (value: string) => {
    ...
    if (normalized === "completed")
      return { label: "Completed", bg: "#14532d", color: "#86efac" };
    if (normalized === "cancelled")
      return { label: "Cancelled", bg: "#7f1d1d", color: "#fecaca" };
    if (normalized === "missed")
      return { label: "Missed", bg: "#1f2937", color: "#cbd5f5" };
    return { label: "Pending", bg: "#78350f", color: "#fde68a" };
  };

  const priorityBadge = (value: string) => {
    const normalized = value.toLowerCase();
    if (normalized === "high")
      return { label: "High", bg: "#7f1d1d", color: "#fecaca" };
    if (normalized === "low")
      return { label: "Low", bg: "#1e3a8a", color: "#bfdbfe" };
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, opacity: 0.75 }}>
              All hospital checkups with filters and actions.
            </div>
          </div>
          <a
            href="/chat/nurse"
      ...
      >
        ...
        >
          {error ? (
            ...
            <div>Loading checkups...</div>
          ) : checkups.length === 0 ? (
            <div>
              {hasFilters
                ? "No checkups match selected filters."
                : "No checkups scheduled."}
            </div>
          ) : (
            <div style={{ width: "100%", overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
                ...
                <tbody>
                  {checkups.map((chk) => {
                    const status = statusBadge(chk.status);
                    const priority = priorityBadge(chk.priority);
                    const actionsDisabled =
                      chk.status === "completed" || chk.status === "cancelled";
                    return (
                      <tr key={chk.checkup_id} style={{ borderTop: "1px solid rgba(148,163,184,0.2)" }}>
                        <td style={{ padding: "12px 8px" }}>{chk.patient_name}</td>
                        <td style={{ padding: "12px 8px" }}>{chk.doctor_name}</td>
                        <td style={{ padding: "12px 8px" }}>{chk.department}</td>
                        <td style={{ padding: "12px 8px" }}>{chk.reason ?? "N/A"}</td>
                        <td style={{ padding: "12px 8px" }}>{chk.date}</td>
                        <td style={{ padding: "12px 8px" }}>{chk.time}</td>
                        ...
                        <td style={{ padding: "12px 8px" }}>
                          {chk.report_url ? (
                            <a
                              href={`${API_BASE}${chk.report_url}`}
                              target="_blank"
                              rel="noreferrer"
                              style={{ color: "#27D4CF", textDecoration: "underline" }}
                            >
                              View
                            </a>
                          ) : (
                        ...
                        <td style={{ padding: "12px 8px" }}>
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            <button
                              onClick={() => updateCheckup(chk.checkup_id, "complete")}
                            ...
                            <button
                              onClick={() => updateCheckup(chk.checkup_id, "cancel")}
                              disabled={actionsDisabled}
                              style={{
                                padding: "6px 10px",
                                borderRadius: 999,
                            ...
                            <button
                              onClick={() => updateCheckup(chk.checkup_id, "priority_high")}
                              style={{
                                padding: "6px 10px",
                                borderRadius: 999,
